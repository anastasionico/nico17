  ...
  */
	...
	$.ui.plugin.add("resizable", "animate", {
		...
		stop: function stop(event, ui) {
			...
			self.element.animate($.extend(style, top && left ? { top: top, left: left } : {}), {
...
(function ($, undefined) {
	...
	$.widget("ui.autocomplete", {
		...
		_create: function _create() {
			...
			this._initSource();
			this.menu = $("<ul></ul>").addClass("ui-autocomplete").appendTo($(this.options.appendTo || "body", doc)[0])
			...
			.css({ top: 0, left: 0 }).hide().data("menu");
			if ($.fn.bgiframe) {
...
(function ($) {
	...
	$.widget("ui.menu", {
		...
		activate: function activate(event, item) {
			this.deactivate();
			if (this.hasScroll()) {
		...
		move: function move(direction, edge, event) {
			if (!this.active) {
